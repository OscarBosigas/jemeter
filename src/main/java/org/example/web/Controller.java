...
public class Controller {
    ...
    @GetMapping("/task")
    public ResponseEntity<TaskDto> taskById(@RequestParam int id){
        try {
    ...
    @PostMapping("/upload")
    public ResponseEntity upload(@RequestBody TaskDto taskDto1) throws IOException {
        TaskDto taskDto = TaskDto.builder().time(LocalDateTime.now())
                .fileName(taskDto1.getFileName()).email(taskDto1.getEmail())
                .status("uploaded").format(taskDto1.getFormat()).build();
        ...
        return ResponseEntity
                ...
                .header("Content-Disposition", "attachment; filename=" + taskDto.getFileName() + "." + taskDto1.getEmail()).build();
    }
